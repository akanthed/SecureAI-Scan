...
**What's a known, not-yet-fixed false positive:** two `SKL003` hits remain on `awslabs/mcp` skills whose own names ("distributed postgres", "distributed sql") don't share a token with "dsql", so fix #4 above doesn't reach them — same underlying pattern (a companion-reference-file pointer, "**When:** Always load for guidance using or updating the DSQL MCP server", the standard progressive-disclosure convention Anthropic's own skill-authoring guidance recommends), just not caught by the narrower fix. Five `MCP009` hits are the same shape one level up: AWS's own tool docstrings comparing two sibling tools it owns ("## When to Use Transact vs readonly_query — Use `readonly_query` for single read queries..."), which is ordinary API documentation, not the redirect-toward-self language the rule's threat model actually targets (confirmed against the existing `send_email`/`format_text` vulnerable fixture, which *is* a same-file attack — so the fix isn't "exclude same-file," it's "distinguish disambiguation from redirection," which needs more care than this pass gives it). Left unfixed rather than rushed, and left out of the findings above rather than reported as real. Tracked as a follow-up, not baselined as accepted.
