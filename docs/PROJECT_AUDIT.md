...
### 2.4 2.8 MB of PNGs committed to git history for content excluded from the npm tarball — 🔧 Partially resolved
**Effort: S**
`mcp-attack-diagram.png` (1.36 MB) and `rag-poisoning-diagram.png` (1.48 MB) are tracked in git. They're correctly excluded from the npm package via `.npmignore`, so they don't bloat installs — but they permanently bloat `git clone` size and repo history for every future contributor. Large binary assets like this belong either compressed, hosted externally (GitHub release assets, a CDN, or the repo's own GitHub Pages `docs/`), or added via Git LFS — not committed raw to the default branch.

Both were flat-color diagrams (no photographic content) saved as unoptimized truecolor PNGs. Losslessly re-encoded as palette PNGs at max compression — verified visually identical, sizes cut roughly in half (1.30MB→0.68MB, 1.41MB→0.69MB). This reduces the ongoing cost of keeping them tracked but does not remove the older, larger blobs already in git history — a full fix would mean rewriting history, which stays a maintainer decision, not something to do silently. Separately, the `demo.gif` (24.2 MB) that a prior session left untracked and referenced from the README is no longer present on disk; the README's broken image reference to it has been removed and replaced with a placeholder comment so nothing renders broken on GitHub in the meantime.
